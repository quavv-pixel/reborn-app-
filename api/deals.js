// This week's grocery ad specials — Vercel serverless function at
// /api/deals.
//
// The browser never talks to Anthropic directly. Calling from the client
// with no auth header would either fail (401) or ship the key to every
// visitor, so the call lives here, server-side, and ANTHROPIC_API_KEY stays
// on the server.
//
// Claude answers this with the web_search tool, so the prices come from
// searching current ads rather than from the model's memory. That is the
// only reason this endpoint is worth having — a model asked for grocery
// prices without search would invent plausible ones.
//
// Not limited to one chain: the prompt searches across whatever grocery
// stores actually serve the given area, and each result carries which store
// it came from.
//
...
// A hard cap needs a shared store (Vercel KV, Upstash).
...
function parseDeals(text) {
  ...
  return parsed
    ...
    .map(d => ({
      ...
      store: d.store ? String(d.store).slice(0, 40) : "",
    }));
}
...
export default async function handler(req, res) {
  ...
  const area = cleanArea(req.body?.store);
  if (!area) return res.status(400).json({ error: "Tell me roughly where to look." });

  const client = new Anthropic({ apiKey });

  const messages = [{
    role: "user",
    content: `Search the web for this week's grocery weekly-ad specials near ${area}. Don't limit this to one chain — search across whatever grocery and supermarket chains actually serve that area (for example Aldi, Walmart, Kroger, Publix, Safeway, Target, Trader Joe's, Meijer, or other local grocers, whichever are relevant there) and pull specials from several of them if you can find them. Return ONLY a raw JSON array, no markdown fences and no preamble, of up to ${MAX_DEALS} objects with keys "item" (short product name), "price" (string like "$2.49"), "note" (a few words, e.g. "per lb" or "Sun-Sat only"), and "store" (the name of the store/chain the deal is from, e.g. "Aldi" or "Kroger"). Only include items you actually found in a current ad — if you cannot find current specials, return [].`,
  }];
