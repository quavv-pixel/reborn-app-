...
`npm test` runs the rotation and list-building tests (29 of them, no framework).

## Deploy it

It's a Vite app with one serverless function, which Vercel handles natively.
...
2. Set **Root Directory** to `skinny-week`. This is the only non-default setting
   — Vercel then auto-detects Vite and picks up `api/deals.js` as a function.
3. Deploy.

Everything except the ALDI ad works immediately with no keys and no
configuration: the rotation, the picking, the shopping list, "Copy for Claude",
and the cooked-recipe memory all run client-side.
...
test/rotation.test.js      29 tests over rotation.js and the book data
```
